    ...
    <body>
                ...
                <form class="layui-form" id="uploadForm">
				  <div class="layui-form-item" style="width: 70%;">
				    <label class="layui-form-label">视频标题</label>
				      <div class="layui-input-block">
				        <input type="text" name="title" required  lay-verify="required" maxlength="50" placeholder="请输入视频标题" autocomplete="off" class="layui-input">
				     </div>
				  </div>
				  <div class="layui-form-item layui-form-text" style="width: 70%;">
			       ...
			       <div class="layui-form-item">
			           ...
			           <div class="form-group" style="width: 560px; margin-left: 110px;">
			               <div id="dropz" class="dropzone"></div>
			           </div>
			           <div id="imgs" style="padding: 5px 10px 10px 5px;text-align: center;"></div>
			           <span id="picture_Id"></span>
			       </div>

			       <div class="layui-form-item">
			           <label class="layui-form-label">视频</label>
			           <div class="form-group" style="width: 560px; margin-left: 110px;">
			               <div id="dropzs" class="dropzone"></div>
			           </div>
			           <div id="imgs" style="padding: 5px 10px 10px 5px;text-align: center;"></div>
        ...
        <script>
		    ...
		    var myDropzone = new Dropzone("#dropz", {
		        ...
		        dictDefaultMessage:'拖动要上传视频截图到此',
		        dictMaxFilesExceeded: "只能上传一张视频截图",
		        ...
		        init:function(){
		            ...
		            this.on("success",function(file,data){
		                ...
		                console.log('ok');
		                document.querySelector('div .dz-success-mark').style.display = 'block';
		                $("#picture_Id").append("<input type='hidden'"+" name='upload_picture_id'" + "value=" + data.id + ">");
		    ...
		    //视频上传
		    var myDropzoneVideo = new Dropzone("#dropzs", {
		        headers: { 'X-CSRF-TOKEN' : '{{ csrf_token() }}' },
		        autoProcessQueue: true, //自动上传
		        url: "/admin/upload",//文件提交地址
		        type:"post",  
		        paramName:"video", //默认为file
		        maxFiles:1,//一次性上传的文件数量上限
		        maxFilesize: 1024, //文件大小，单位：MB
		        acceptedFiles: ".avi,.mp4,.rmvb,.mkv,.webm", //上传的类型
		        uploadMultiple: true,
		        addRemoveLinks:true,
		        parallelUploads: 1,//一次上传的文件数量
		        dictDefaultMessage:'拖动要上传的视频到此',
		        dictMaxFilesExceeded: "您最多只能上传1个视频！",
		        dictResponseError: '文件上传失败!',
		        dictFallbackMessage:"浏览器不受支持",
		        dictFileTooBig:"文件过大上传文件最大支持.",
		        dictRemoveFile: '删除',
		        ...
		        init:function(){
		            ...
		            this.on("success",function(file,data){
		                ...
		                console.log('ok');
		                document.querySelector('div .dz-success-mark').style.display = 'block';
		                $("#video_Id").append("<input type='hidden'"+" name='upload_video_id'" + "value=" + data.id + ">");
		                $('.dz-remove').remove('href');
		                $("#dropzs a:first").attr('onclick',"video_del(this,'"+ data.id +"')");
